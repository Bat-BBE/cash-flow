...
import { ACCOUNTS, CATEGORIES } from "@/hook/use-transaction-data";
...
function titleForPath(p: string | null): { title: string; icon: string } {
  ...
  return {
    title: PAGE_TITLE[key] ?? "CashFlow",
    icon: PAGE_ICONS[key] ?? "circle",
  };
}
...
const INITIAL_NOTIFS: NotifItem[] = [
  {
    ...
    desc: "Энэ сарын орлого өмнөхөөс 12% өсчээ.",
    time: "2 цагийн өмнө",
    read: false,
  },
  {
    ...
    bg: "bg-amber-500/20",
    title: "Зарлагын лимит",
    desc: "Хоол хүнсний зарлага лимитийн 85%-д хүрлээ.",
    time: "5 цагийн өмнө",
    read: false,
  },
  {
    ...
    desc: "Маргааш интернетийн төлбөр хийгдэнэ.",
    time: "Өчигдөр",
    read: false,
  },
  {
    ...
    bg: "bg-violet-500/20",
    title: "Зорилго биелэлт",
    desc: "Хадгаламжийн зорилгын 60%-д хүрлээ.",
    time: "2 өдрийн өмнө",
    read: true,
  },
  {
...
function NotifPanel({
  ...
  const panelContent = (
    <>
        ...
        <div className="flex items-center gap-2 sm:gap-2.5">
          ...
          <span
            className={cn(
              ...
              isMobile ? "text-[12px]" : "text-[13px] font-black",
            )}
          >
            Мэдэгдэл
          </span>
      ...
      {/* ── List ── */}
      <div
        className={cn("overflow-y-auto", isMobile && "min-h-0 flex-1")}
        ...
        {notifs.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-12">
            <div className="w-12 h-12 rounded-2xl bg-white/[0.04] border border-white/[0.07] flex items-center justify-center">
              <span className="material-symbols-outlined text-2xl text-white/15">
                notifications_off
              </span>
            </div>
      ...
      >
        ...
        <div className="py-1">
          {[
            ...
            >
              <span className="material-symbols-outlined text-[17px] text-violet-400/70">
                {icon}
              </span>
              {label}
            </button>
...
export function Header({
  ...
  const unread = notifs.filter((n) => !n.read).length;
  const dataCtx = useDashboardDataOptional();
  const accList = accounts.length > 0 ? accounts : ACCOUNTS;
  const catList = categories.length > 0 ? categories : CATEGORIES;

  const handleAddFromModal = async (tx: unknown) => {
    if (onAddTransaction) {
      onAddTransaction(tx);
      return;
    }
    const p = tx as Record<string, unknown>;
    if (p.type === "file_import" && p.file instanceof File) {
      try {
        const r = await dataCtx?.uploadTransactionFile(p.file);
        if (r) {
          const extra =
            r.warnings?.length > 0 ? `\n\n${r.warnings.join("\n")}` : "";
          window.alert(
            `${r.imported} гүйлгээ амжилттай нэмэгдлээ.${extra}`,
          );
        }
      } catch (e) {
        console.error(e);
        window.alert(
          e instanceof Error
            ? e.message
            : "Файл унших эсвэл хадгалахад алдаа гарлаа.",
        );
      }
      return;
    }
    if (!dataCtx) return;
    try {
      await dataCtx.addTransaction({
        name: String(p.name || p.description || p.category || "Гүйлгээ"),
        date: String(p.date),
      ...
      console.error(e);
      window.alert("Гүйлгээ хадгалахад алдаа гарлаа.");
    }
  };
        ...
        <div className="flex h-[3.75rem] items-center gap-1.5 px-2.5 sm:gap-2 md:h-[4.25rem] md:gap-2 md:px-5">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-1.5 sm:gap-3">
            ...
            <div className="hidden shrink-0 items-center gap-2.5 md:flex">
              ...
              />
              <p className="text-[11px] font-black uppercase tracking-[0.2em] text-violet-400/80">
                CashFlow
              </p>
            </div>
            ...
            <div className="hidden md:flex items-center gap-2">
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-white/[0.05] border border-white/[0.07]">
                <span className="material-symbols-outlined text-[15px] text-white/50">
                  {pageIcon}
                </span>
              </div>
              <h1 className="text-[13px] font-bold text-white/70 tracking-wide">
                {pageTitle}
              </h1>
            </div>
            ...
            >
              <img
                src="/logo.png"
                alt=""
                className="h-10 w-8 shrink-0 object-contain opacity-95"
              />
              <span className="hidden min-[360px]:inline text-[10px] font-black uppercase tracking-[0.2em] text-violet-400/75">
                CashFlow
              </span>
            </Link>
            <div className="md:hidden flex min-w-0 items-center gap-1.5">
              ...
              <span className="truncate text-[11px] font-semibold leading-tight text-white/45">
                {pageTitle}
              </span>
            </div>
          </div>
          ...
          <div className="flex shrink-0 items-center gap-1 md:gap-2">
            ...
            >
              ...
              <span className="material-symbols-outlined relative z-10 text-[16px] leading-none md:text-[15px]">
                add
              </span>
            ...
            <div ref={notifRef} className="relative">
              <button
                ...
                className={cn(
                  ...
                  showNotif &&
                    "border-white/[0.12] bg-white/[0.07] text-white/75",
                )}
                aria-label="Мэдэгдэл"
              >
                <span className="material-symbols-outlined text-[16px] leading-none md:text-[17px]">
                  notifications
                </span>
                {unread > 0 && (
