...
function TierBadge({ tier, size = 'sm' }: { tier: string; size?: 'xs' | 'sm' }) {
  return (
    ...
    >
      <svg className={size === 'xs' ? 'w-2 h-2' : 'w-2.5 h-2.5'} viewBox="0 0 12 12" fill="currentColor">
...
/* ── Stat card ── */
function StatCard({
  label, value, sub, subColor, bar, barColor, icon, ring, ringMax, ringColor,
}: {
  label: string; value: string | number; sub?: string; subColor?: string;
  bar?: number; barColor?: string; icon?: string;
  ring?: number; ringMax?: number; ringColor?: string;
}) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-white/[0.07] bg-white/[0.03] p-4 transition-all duration-300 hover:border-white/[0.12] hover:bg-white/[0.05]">
      ...
      <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
        style={{ background: 'radial-gradient(ellipse at top left, rgba(251,191,36,0.04), transparent 60%)' }} />

      <div className="mb-3 flex items-start justify-between gap-2">
        <span className="text-[9px] font-black uppercase tracking-[0.18em] text-white/25">{label}</span>
        {ring !== undefined ? (
          <ProgressRing value={mounted ? ring : 0} max={ringMax ?? 100} color={ringColor ?? '#10b981'} size={44} />
        ) : icon && (
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-white/[0.05] transition-colors group-hover:bg-white/[0.08]">
            <span className="material-symbols-outlined text-sm text-white/30 group-hover:text-white/50">{icon}</span>
          </div>
        )}
      </div>
      ...
      {bar !== undefined && (
        <div className="mt-3 h-[2px] w-full overflow-hidden rounded-full bg-white/[0.05]">
          <div
            className={cn('h-full rounded-full transition-all duration-1000 ease-out', barColor ?? 'bg-emerald-500')}
      ...
      {sub && (
        <p className={cn('mt-1.5 text-[10px] font-bold', subColor ?? 'text-white/35')}>{sub}</p>
      )}
    </div>
  );
}
...
/* ── Badge item ── */
function BadgeCard({ name, icon, earned, color }: { name: string; icon: string; earned: boolean; color: string }) {
  ...
  return (
    ...
    >
      <div
        ...
        style={earned && hovered ? { boxShadow: '0 8px 24px rgba(0,0,0,0.4)' } : {}}
      >
        {earned && (
          <div className="absolute inset-0 rounded-2xl opacity-40"
            style={{ background: 'radial-gradient(circle at 35% 35%, rgba(255,255,255,0.25), transparent 65%)' }} />
        )}
        <span className="material-symbols-outlined text-xl text-white relative z-10">{icon}</span>
      </div>
      <p className={cn('text-[10px] font-black text-center leading-tight', earned ? 'text-white/80' : 'text-white/20')}>
        {name}
      </p>
      {!earned && <span className="text-[8px] font-bold text-white/20 tracking-wide">ОЛДООГҮЙ</span>}
    </div>
  );
}
...
function ActivityItem({
  action, time, icon, colorClass, index,
}: { action: string; time: string; icon: string; colorClass: string; index: number }) {
  ...
  return (
    ...
    >
      <div className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border', colorClass)}>
        <span className="material-symbols-outlined text-[14px]">{icon}</span>
      </div>
      <div className="min-w-0 flex-1">
        <p className="truncate text-[12px] font-semibold text-white/75">{action}</p>
...
/* ── Stat row ── */
function StatRow({
  label, value, icon, color, bg, border, index,
}: { label: string; value: string; icon: string; color: string; bg: string; border: string; index: number }) {
  const [visible, setVisible] = useState(false);
  useEffect(() => {
    const t = setTimeout(() => setVisible(true), index * 80);
    return () => clearTimeout(t);
  ...
  return (
    ...
    >
      <div className={cn('flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border', bg, border)}>
      ...
      <span className={cn('material-symbols-outlined text-sm', color)}>chevron_right</span>
    </div>
  );
}
    ...
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      ...
      >
        ...
        <div className="relative h-28 shrink-0 overflow-hidden sm:h-32 md:h-[8.5rem]">
          ...
          {/* top golden bloom */}
          <div className="absolute -top-8 left-1/2 h-48 w-64 -translate-x-1/2 rounded-full opacity-25 blur-3xl"
            style={{ background: 'radial-gradient(circle, #f59e0b 0%, transparent 70%)' }} />
          {/* bottom violet */}
          <div className="absolute bottom-0 right-0 h-32 w-48 rounded-full opacity-20 blur-2xl"
          ...
          >
            <span className="material-symbols-outlined text-[16px] text-white/40 transition-colors group-hover:text-white/80">close</span>
          </button>
        </div>
        ...
        <div className="relative z-20 shrink-0 px-4 max-md:px-4 md:px-8">
          ...
          <div className="absolute -top-9 left-1/2 z-20 -translate-x-1/2 max-md:left-5 max-md:translate-x-0 md:-top-[4.75rem]">
            <div className="relative">
              {/* glow ring */}
              <div className="absolute -inset-[3px] rounded-[22px] opacity-60 blur-md"
                style={{ background: 'linear-gradient(135deg, rgba(251,191,36,0.5), rgba(124,58,237,0.3))' }} />
        ...
        >
          ...
          <p className="mt-1.5 text-sm font-medium text-white/30">@{u.username}</p>
          <div className="mt-2.5 flex items-center gap-1.5 text-[11px] font-medium text-white/20 md:justify-center">
            <span className="material-symbols-outlined text-[13px] text-white/20">calendar_today</span>
          ...
          <div className="space-y-3 p-4 md:space-y-4 md:px-8 md:pb-8">
            ...
            {activeTab === 'Тойм' && (
              <>
                ...
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.025] p-4">
                  <p className="mb-2.5 text-[9px] font-black uppercase tracking-[0.18em] text-white/20">Тухай</p>
                  <p className="text-[12.5px] leading-relaxed text-white/55">{u.bio}</p>
                </div>
                ...
                <div className="flex items-center gap-3.5 rounded-2xl border border-white/[0.06] bg-white/[0.025] p-4">
                  ...
                  <div>
                    <p className="text-[9px] font-black uppercase tracking-[0.15em] text-white/20">Гишүүн болсон</p>
                    <p className="mt-0.5 text-[13px] font-bold text-white/75">{u.joinedDate}</p>
                  </div>
                </div>
              </>
            )}
            ...
            {activeTab === 'Статистик' && (
              <div className="space-y-2.5">
                {[
                  { label: 'Нийт гүйлгээ',     value: '1,240',   icon: 'receipt_long',          color: 'text-sky-400',     bg: 'bg-sky-500/[0.08]',     border: 'border-sky-500/20'     },
                  { label: 'Дундаж зарлага',    value: '₮320к',   icon: 'trending_down',         color: 'text-orange-400',  bg: 'bg-orange-500/[0.08]',  border: 'border-orange-500/20'  },
                  { label: 'Хамгийн их орлого', value: '₮1.2сая', icon: 'trending_up',           color: 'text-emerald-400', bg: 'bg-emerald-500/[0.08]', border: 'border-emerald-500/20' },
                  { label: 'Идэвхтэй өдрүүд',  value: '218',     icon: 'local_fire_department', color: 'text-rose-400',    bg: 'bg-rose-500/[0.08]',    border: 'border-rose-500/20'    },
                ].map((item, i) => (
            ...
            {activeTab === 'Үйл ажиллагаа' && (
              <div className="space-y-0.5">
                {[
                  { action: 'Шинэ зорилго нэмсэн',   time: '2 цагийн өмнө', icon: 'flag',       colorClass: 'text-indigo-400 bg-indigo-500/[0.08] border-indigo-500/20' },
                  { action: 'Гүйлгээ хийсэн',         time: '5 цагийн өмнө', icon: 'swap_horiz', colorClass: 'text-sky-400 bg-sky-500/[0.08] border-sky-500/20'         },
                  { action: 'Профайл шинэчилсэн',     time: 'Өчигдөр',       icon: 'edit',       colorClass: 'text-violet-400 bg-violet-500/[0.08] border-violet-500/20' },
                  { action: 'Системд нэгдсэн',        time: u.joinedDate,    icon: 'login',      colorClass: 'text-amber-400 bg-amber-500/[0.08] border-amber-500/20'   },
                ].map((item, i) => (
